...
const knockoutMovementPattern = /\b(advance[sd]?|advanced|through|go through|move on|moving on|ticket|out|gone|go out|end of the road|road (?:closed|ends|is closed)|closed (?:that road|the door)|run out of road|stops? here|sent .* home|kept the ticket|quarterfinals?|semifinals?|final|next round|moved on|survive|survives|send .* home|knocked .* out)\b/i;

const storyTexturePattern = /\b(receipts?|witness(?:es)?|chat|argument|louder|quiet|brave|spotlight|lawyer|oxygen|ticket|road|picks? aged|personal|material|speak first|old picks|closed the door|breathe|breathing|no longer|audience|souvenir|bandage|smile|chair|water|proof|proof point|sweat|almost|alive|explanation|pulse|lift|swing|shake|moment)\b/i;
...
function scoreOutcome(match, story) {
  if (story && story.outcome) return story.outcome;
...
for (const story of latest) {
  ...
  const outcome = scoreOutcome(match, story);
