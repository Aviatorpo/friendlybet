...
vm.runInContext(`${phaseSource}; ${phaseOfficialSource}; this.groupStagePhase = _groupStagePhase; this.phaseHasOfficialScoring = _phaseHasOfficialScoring;`, sandbox);
...
assert(
  /renderTheoreticalLeaderboard\(users,\s*\{\s*\.\.\.options,\s*hasScores,\s*phase\s*\}\)/.test(leaderboard),
  'Leaderboard must pass official-score and phase state into the theoretical table renderer'
);
...
assert(
  /options\.hasScores\s*\|\|\s*_phaseHasOfficialScoring\(options\.phase\)/.test(theoretical),
  'Theoretical table must not render after official scoring starts or groups complete'
);
