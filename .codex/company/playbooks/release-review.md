...
0a. For any user-visible bug or fix request, treat production release and live proof as the default outcome unless Eyal explicitly requested local-only, plan-only, or draft-only work. Do not close on "implemented locally"; keep the incident open until live-verified or until the exact blocked layer is stated.
...
7. For live user-visible data/content/app fixes, verify the deployed production artifact with a cache-busting URL after push. For example: `https://friendlybet.live/app.js?cb=<timestamp>`, `https://friendlybet.live/public-data/world-cup-stories.json?cb=<timestamp>`, or `https://friendlybet.live/public-data/pundit.json?cb=<timestamp>`.
8. If the production artifact has not updated yet, report "pushed but not live", name the deploy/cache/service-worker layer still unverified, and keep the incident open.
