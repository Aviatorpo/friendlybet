...
- Local renderer floor: `scripts/process-story-image.py outcome-base` is production-critical, not a harmless fallback. It must render named-player, numbered-kit poster bases with a clean caption-safe band; it must never regress to abstract shape cards, duplicate outcome text, or team-name-only placeholders.
...
- Pool phase anchoring: pool-specific Pundit copy must speak from the user's current tournament phase. After the group stage is complete and knockout fixtures are live or finished, do not describe members as "leaving the group stage", "after the group stage", or receiving "group-stage receipts" as the current story. Anchor pool lines to the current leaderboard, locked bracket, knockout picks, verified results, and what can still change.
- Phase-copy regression: any app-code change touching `buildPoolPundit()` must run `node scripts\test-pundit-client-staleness.js`. The test must block stale phase-transition phrases in client-generated pool copy, because this content can bypass `public-data/pundit.json` refreshes.
...
Reject copy that treats a completed tournament phase as if the user is still transitioning through it. Historical context is allowed only when the sentence immediately returns to the current user job: current leaderboard, current lock state, current knockout result, current scoring impact, or next known fixture.
