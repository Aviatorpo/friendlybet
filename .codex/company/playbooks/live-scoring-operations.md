...
- Consensus policy for automatic scoring: official FIFA final/advancement can verify alone when primary sources do not contradict it; otherwise require strong corroboration from approved independent reliable source families, normally all available sources agreeing or at least three independent families agreeing on score/status/advancing team. If primary sources such as FIFA and ESPN explicitly disagree, the verifier must immediately check the approved emergency source shelf in the same run. Three additional non-primary independent source families agreeing with each other can override the primary conflict and score the match; fewer than three leaves the match in automatic retry/review without a hard attention failure. If fewer non-conflicting sources are available in non-conflict cases, require at least two approved independent source families plus no contradiction.
