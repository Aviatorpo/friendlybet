...
- Consensus policy for automatic scoring: official FIFA final/advancement can verify alone when complete and clean; otherwise require strong corroboration from approved independent reliable source families, normally all available sources agreeing or at least three independent families agreeing on score/status/advancing team. FIFA and ESPN are primary signals, not a blocking gate. If the primary pair explicitly disagrees, one primary source is missing, stale, incomplete, unparseable, returns no matching fixture, returns a final without score, or otherwise cannot produce a scoreable final, the verifier must immediately check the approved emergency source shelf in the same run. Three additional non-primary independent source families agreeing with each other can resolve the match; fewer than three leaves the match in automatic retry/review without a hard attention failure. If fewer non-conflicting sources are available in non-primary cases, require at least two approved independent source families plus no contradiction.
- football-data.org is not an approved final-result source. It may not be added to `RESULT_FALLBACK_SOURCES`, used to set `winner_code`, or used to turn a match scoreable. Legacy football-data sync scripts must fail closed unless explicitly enabled for isolated legacy testing/recovery.
- Approved WC2026 result source shelf: default automatic path is FIFA + ESPN. Emergency verifier mode may also use LiveScore match center (`scoreboard:livescore`), FOX Sports World Cup scores (`scoreboard:fox`), Yahoo Sports scoreboard/articles (`scoreboard:yahoo`), Guardian Content API/articles (`media:guardian`), AP News search/articles (`wire:ap`), and operator-supplied source URLs from Houston Chronicle or NY Post (`media:houston-chronicle`, `media:nypost`). These sources can corroborate only through the final-result verifier, not by direct DB edits. Scheduled runs must not scrape every emergency source by default.
- Rejected/unproven source shelf: BBC and Sky were not yet proven parseable for current WC2026 automated final-result rows; Reuters blocked server-side access; SofaScore returned 403; FotMob public paths tested as 404; 365Scores/CBS did not expose reliable match rows in the checked pages. Do not add any of these to scoring without a fresh connector proof, deterministic parser test, cost/rate-limit review, and QA sign-off.
...
- Any code path that changes a scoreable terminal fact in Supabase `matches` changes the canonical `result_version` and must enqueue or run scoring, leaderboard export, and public proof. This includes the final verifier, live poller, FIFA schedule bridge, break-glass repair, and correction paths. A fresh `matches.json` without same-version scored leaderboards is a red incident, not a partial success.
...
- Current WC2026 final-result ladder: Plan A = FIFA official final/advancement writes the canonical result. Plan B = live full-time detection wakes the same verifier immediately and scores only after ESPN+FIFA or approved consensus confirms. Plan C = scheduled verifier plus stored source-observation ledger replays approved non-football-data source-family evidence and resumes from Supabase checkpoints. Plan D = safe-wait: keep the match in result-confirmation state, do not score, keep polling bounded approved sources, and replay scoring/publication automatically when evidence arrives.
- Emergency source-family escalation must be automatic in scheduled final-result verifier and live-poller final handoff. Manual `emergency_sources` workflow input is allowed for diagnostics, but it is not the product fallback. The verifier decides from stuck-match age/stale-live state, opens only the approved source registry, and still requires consensus plus no official contradiction.
...
- The scheduled verifier should cover normal missed finals and older tournament residue with bounded backoff. Non-terminal stale-live rows, missing scores, and tied knockouts without verified advancement remain recovery candidates. Already scored `FINISHED`/`AWARDED` rows with stale live residue are cleanup/snapshot-sanitization work, not final-result truth work.
...
- Treat the June 30 failure pattern as a standard rehearsal: two penalty-decided knockouts, manual truth pressure from Eyal, Action failure noise, stale public artifact shape, too-short CDN proof, and stale live residue misclassified as result work. Future plans must show why those exact failures cannot recur.
...
- Expected-score guard: for scoring incidents, DB-vs-public-snapshot equality is not enough. Recompute the expected score components for the affected prediction mode from canonical matches and picks, then compare expected values to Supabase before saying pools are fully updated. For two-phase knockout scoring, explicitly prove the FIFA slot bridge maps from `buildGroupState()` output to synthetic pick ids such as `R32_M1`.
...
- Result-version publication is broader than numeric score changes. After any verified final creates a new `result_version`, regenerate and prove all static leaderboard snapshots for that version, even when `changed_pool_ids` is empty or only some pools had score deltas.
- Source-to-screen publication proof must reject split versions: Supabase `matches`, all score rows that existed before the latest result update, public match snapshots, and public leaderboard snapshots must agree that the latest `result_version` has been scored. Readiness may not go green while match snapshots are newer than score publication.
- Public match snapshots must de-duplicate logical fixtures before publication. Prefer official/FIFA enriched rows, verified finals over provider-pending rows, and newer trusted timestamps so stale legacy provider rows cannot win display order or result context.
...
- Automatic acceptance: accept FIFA final/advancement when complete and clean, or independent multi-source consensus on the advancing team and final status. If FIFA/ESPN disagree or either primary source cannot produce a usable scoreable final after being checked, automatically escalate to the emergency source shelf and accept the result only when three additional non-primary source families agree with each other. Prefer exact score agreement, but do not block scoring if every trusted source agrees on who advanced and only penalty-shootout details are missing.
- Automatic retry/review: keep polling and escalating approved sources when fewer than three additional non-primary families agree after primary-source trouble, sources disagree on the advancing team, the teams do not match the fixture, winner is not one of the teams, a postponed/suspended/cancelled state appears, a decisive score contradicts the claimed winner, or the provider row is stale. Primary-source trouble alone must not become a hard Action failure while fallback consensus is still possible.
...
- Sources: FIFA complete result verifies first. If FIFA is late or incomplete, use approved independent source-family consensus that excludes football-data. If automation cannot resolve the result, keep polling/escalating approved sources with bounded backoff. If uncertainty remains, hold scoring and show the result-confirmation state.
...
- Run the deterministic result/scoring tests: `test-scoring`, `test-espn-live-sync`, `test-final-result-verifier-needed`, `test-final-result-verifier`, `test-final-result-verifier-auto-emergency`, `test-export-snapshots`, `test-verify-scoring-snapshots`, `test-live-poller`, and `test-banter`. `test-scoring` must cover partial group completion, pending-provider finals, single-phase group positions, two-phase advancers, third-place wait-until-all-12, penalties, multipliers, and late-knockout pools.
