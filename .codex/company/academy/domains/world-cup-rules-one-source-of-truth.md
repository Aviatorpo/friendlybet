...
| Knockout matches level after normal time go to extra time and then penalties if still level. | FIFA World Cup 26 Regulations, Article 14 | Reinforces using verified advancement, stored as resolved `matches.winner_code`, rather than score comparison for knockout scoring. | QA must keep extra-time, penalty, and tied-score advancement tests. |
...
| Knockout winner must come from verified advancement, stored as the resolved `matches.winner_code`, not score comparison. `winner_code` is an output of source consensus/rule validation, not a raw truth source. | `scripts/calculate-scores-v2.js`, app data model | `scripts/test-scoring.js` | Engineering and QA must block score-only winner inference and also block stale, contradictory, or unverified `winner_code` values. |
