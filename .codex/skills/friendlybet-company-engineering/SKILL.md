...
14a. For scoring correctness incidents, do not stop at proving public snapshots match Supabase. Add or run a deterministic expected-score audit that recomputes the affected component from canonical matches and picks. For two-phase knockout scoring, explicitly prove the FIFA slot bridge maps `buildGroupState()` output to synthetic pick ids such as `R32_M1`.
14b. Treat `result_version` as a publication contract, not just a score-change artifact. After a verified final, regenerate/prove all leaderboard snapshots for the new version even if `changed_pool_ids` is empty or only some pools changed numerically.
