...
- If an image-generation tool rejects a named-player or public-figure prompt, treat that generation route as blocked. Do not silently rewrite the prompt to fictional players, generic stars, anonymous athletes, or lower-standard artwork. Use approved local named-player assets as references/compositing sources, get an explicitly approved alternate generation path, or stop for manual review.
...
- Confirm any image-tool safety refusal did not become a hidden standards downgrade. If the final asset came from a fallback route, record the route and prove the visible players still come from named, verified, approved sources.
