...
- Social texture: use language such as receipts, witnesses, chat arguments, quiet picks getting loud, tickets, roads closing, picks aging in public, or similar.
...
1. Replace weak latest-story English templates in `scripts/generate-world-cup-stories.js`.
...
5. If generation, image audit, story validation, or tone validation fails, do not publish broken optional content.
...
Story charm is important, but it must never block verified results, scoring, leaderboard snapshots, locking, or match display. The Story publisher is optional content automation with release gates.
