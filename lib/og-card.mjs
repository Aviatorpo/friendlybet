...
export function buildCardElement(data) {
  ...
  return h('div', {
    ...
    // title
    h('div', { display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 4 }, [
      h('div', { color: INK, fontSize: 36, fontFamily: 'Sora', fontWeight: 800, display: 'flex' }, 'My Road to Glory'),
      h('div', { color: MUTED, fontSize: 16, display: 'flex' }, 'World Cup 2026 · my bracket'),
    ]),

    // bracket: semis row + final row. Heights/gaps tuned (v2.7.7) so the total
    // column never exceeds 630px — otherwise the champion box's `marginTop:auto`
    // collapsed and the box rode UP over the finals chips (the overlap bug).
    h('div', { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, marginTop: 8 }, [
      sectionLabel('SEMI-FINALS'),
      h('div', { display: 'flex', gap: 12 }, [chip(semis[0], lang, 144), chip(semis[1], lang, 144), chip(semis[2], lang, 144), chip(semis[3], lang, 144)]),
      h('div', { display: 'flex', alignItems: 'center', gap: 14, marginTop: 2 }, [
        chip(finals[0], lang, 152),
        h('div', { color: GOLD, fontSize: 20, fontFamily: 'Sora', fontWeight: 800, display: 'flex' }, 'vs'),
        chip(finals[1], lang, 152),
      ]),
    ]),

    // champion box (lower part). `marginTop:auto` pushes it toward the bottom and
    // absorbs the slack as a gap ABOVE it — safe now that the content fits.
    h('div', {
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      marginTop: 'auto', gap: 4, padding: '10px 0', borderRadius: 20,
      background: 'rgba(217,180,106,0.13)', border: '3px solid #d9b46a',
    }, [
      h('div', { color: GOLD, fontSize: 13, fontFamily: 'Sora', fontWeight: 800, letterSpacing: 3, display: 'flex' }, 'CHAMPIONS'),
      h('div', { display: 'flex', alignItems: 'center', gap: 16 }, [
        flag(champ, 64, 43),
        txt(champ ? teamName(champ, lang) : '—', { color: GOLD_LT, fontSize: 38, fontFamily: 'Sora', fontWeight: 800 }),
      ]),
      txt(lang === 'he' ? 'האלופה שלי למונדיאל 2026' : 'my predicted world champion', { color: MUTED, fontSize: 14 }),
    ]),
    ...
    h('div', { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 6 }, [
