...
export function buildCardElement(data) {
  ...
  return h('div', {
    ...
    // footer
    h('div', { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 12 }, [
      h('div', { color: GOLD, fontSize: 20, fontFamily: 'Sora', fontWeight: 800, display: 'flex' }, 'friendlybet.live'),
      txt(lang === 'he' ? 'בנה תחזית משלך — חינם' : 'build your own — free', { color: MUTED, fontSize: 16 }),
    ]),
  ].filter(Boolean));
}
...
export async function fetchCardData(u, p, lang) {
  ...
  const [usr, pool, ko, tw] = await Promise.all([
    ...
    q(`knockout_picks?pool_id=eq.${p}&user_id=eq.${u}&bracket_position=gte.25&select=bracket_position,predicted_winner`),
