...
- For any user-visible FriendlyBet bug or fix request, default to production ownership unless Eyal explicitly asks for local-only, plan-only, or draft-only work. Do not final-answer with "implemented locally", "done", or equivalent as completion. Valid completion states are live-verified in production; pushed/deploying with the exact production-unverified layer and next proof; or blocked with the exact missing permission/external state after trying the safe release path.
...
- A code fix for a live user-visible issue is not complete until it is shipped to `main`, Vercel/CDN/service-worker cache state is accounted for, and the exact live surface is verified. Local tests are release gates, not completion.
