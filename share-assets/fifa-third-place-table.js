// FIFA World Cup 2026 Annex C: exact third-place-team placement table.
//
// Keys are the eight third-place groups that advance, sorted A-L.
// Values are the assigned third-place groups in FIFA's published column order:
//   1A, 1B, 1D, 1E, 1G, 1I, 1K, 1L
//
// The app's bracket positions for those columns are:
//   1A=>7, 1B=>13, 1D=>9, 1E=>2, 1G=>10, 1I=>5, 1K=>15, 1L=>8
(function (root) {
  'use strict';

  const SLOT_ORDER = [7, 13, 9, 2, 10, 5, 15, 8];
  const RAW = `
...
`;
  ...
  const table = Object.freeze(Object.fromEntries(
    RAW.replace(/\s+/g, '').split('|').filter(Boolean).map(row => row.split(':'))
  ));

  function keyFor(chosenGroups) {
    const key = [...new Set((chosenGroups || []).filter(Boolean))].sort().join('');
    return key.length === 8 ? key : null;
  }

  function assignment(chosenGroups) {
    const key = keyFor(chosenGroups);
    const row = key ? table[key] : null;
    if (!row || row.length !== SLOT_ORDER.length) return null;
    const out = {};
    SLOT_ORDER.forEach((pos, i) => { out[pos] = row[i]; });
    return out;
  }

  const api = Object.freeze({ table, slotOrder: SLOT_ORDER.slice(), keyFor, assignment });
  root.FB_FIFA_THIRD_PLACE = api;
